...
// دریافت سطوح دسترسی نقش
$permissions_stmt = $pdo->prepare("SELECT section FROM permissions WHERE role_id = ?");
$permissions_stmt->execute([$role_id]);
$existing_permissions = $permissions_stmt->fetchAll(PDO::FETCH_COLUMN);
...
// بروزرسانی اطلاعات نقش و سطوح دسترسی
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $name = $_POST['name'];
    $admin_access = isset($_POST['admin_access']) ? 1 : 0;
    $selected_permissions = isset($_POST['permissions']) ? $_POST['permissions'] : [];
    
    $stmt = $pdo->prepare("UPDATE roles SET name = ?, admin_access = ? WHERE id = ?");
    $stmt->execute([$name, $admin_access, $role_id]);
    
    // حذف سطوح دسترسی قبلی
    $stmt = $pdo->prepare("DELETE FROM permissions WHERE role_id = ?");
    $stmt->execute([$role_id]);
    
    // درج سطوح دسترسی جدید
...
<body>
    <div class="container mt-4">
        ...
        <form method="POST" class="w-50 mx-auto">
            ...
            <input type="text" name="name" value="<?= htmlspecialchars($role['name']) ?>" required class="form-control">
            
            <label class="mt-3">Admin Access:</label>
            <div class="form-check">
                <input type="checkbox" name="admin_access" value="1" class="form-check-input" <?= $role['admin_access'] ? 'checked' : '' ?>>
            ...
            <div class="form-check">
                <?php foreach ($sections as $section): ?>
                    <input type="checkbox" name="permissions[]" value="<?= $section ?>" class="form-check-input" 
                        <?= in_array($section, $existing_permissions) ? 'checked' : '' ?>>
